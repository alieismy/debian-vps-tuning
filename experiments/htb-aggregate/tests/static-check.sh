...
write_managed_fixture() {
  local profile="$1" schema="${2:-4}" version="${3:-0.1.0-rc.14}" port="${4:-200}"
  jq -n \
    --arg profile "$profile" \
    --arg version "$version" \
...
write_managed_fixture debian13-1c2g 4 0.1.0-rc.14 500
if (verify_managed_host_baseline) >/dev/null 2>&1; then
...
jq -n --arg script_sha "$script_sha" \
  '{schema_version:1,script_version:"0.1.0-rc.14",profile:"debian13-1c1g",
...
jq -n '{schema_version:4,script_version:"0.1.0-rc.14",state:"VERIFIED",
...
if ! (
  ...
  managed_script_version='0.1.0-rc.14'
...
jq -n --arg tuning_sha "$fixture_tuning_sha" --arg state_sha "$fixture_state_sha" '
  {schema_version:2,runner_version:"0.3.0",tuning_script:{sha256:$tuning_sha},
   managed_binding:{profile_id:"debian13-1c1g",script_version:"0.1.0-rc.14",
