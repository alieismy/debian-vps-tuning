...
printf '%s\n' boot-a >"$boot_file"
printf '%s\n' '{"schema_version":4,"script_version":"0.1.0-rc.17","state":"VERIFIED","profile":{"id":"debian13-1c1g"},"network":{"port_speed_mbps":200}}' >"$state_file"
...
case "$1" in
  verify) jq -e '.script_version=="0.1.0-rc.17" and .state=="VERIFIED"' "$DVT_STATE_FILE" >/dev/null ;;
...
PROFILE_ID='debian13-1c1g'
case "$1" in
  preflight) [ ! -e "$DVT_STATE_FILE" ] || jq -e '.script_version=="0.1.0-rc.17"' "$DVT_STATE_FILE" >/dev/null ;;
  apply) printf '%s\n' '{"schema_version":4,"script_version":"0.1.0-rc.18","state":"APPLIED","profile":{"id":"debian13-1c1g"},"network":{"port_speed_mbps":200}}' >"$DVT_STATE_FILE" ;;
...
if env DVT_STATE_FILE="$state_file" DVT_BOOT_ID_FILE="$boot_file" \
  ...
  --source-version 0.1.0-rc.19 --target-version 0.1.0-rc.18 \
...
env DVT_STATE_FILE="$state_file" DVT_BOOT_ID_FILE="$boot_file" \
  ...
  --source-profile "$source_profile" --target-profile "$target_profile" \
  --source-version 0.1.0-rc.17 --target-version 0.1.0-rc.18 \
...
jq -e '.script_version=="0.1.0-rc.18" and .state=="VERIFIED"' "$state_file" >/dev/null
...
printf 'rc.18 traffic-budget, migration, and benchmark process checks passed\n'
