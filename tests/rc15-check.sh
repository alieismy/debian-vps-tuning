...
printf '%s\n' boot-a >"$boot_file"
printf '%s\n' '{"schema_version":4,"script_version":"0.1.0-rc.14","state":"VERIFIED","profile":{"id":"debian13-1c1g"},"network":{"port_speed_mbps":200}}' >"$state_file"
...
case "$1" in
  verify) jq -e '.script_version=="0.1.0-rc.14" and .state=="VERIFIED"' "$DVT_STATE_FILE" >/dev/null ;;
...
PROFILE_ID='debian13-1c1g'
case "$1" in
  preflight) [ ! -e "$DVT_STATE_FILE" ] || jq -e '.script_version=="0.1.0-rc.14"' "$DVT_STATE_FILE" >/dev/null ;;
  apply) printf '%s\n' '{"schema_version":4,"script_version":"0.1.0-rc.15","state":"APPLIED","profile":{"id":"debian13-1c1g"},"network":{"port_speed_mbps":200}}' >"$DVT_STATE_FILE" ;;
...
env DVT_STATE_FILE="$state_file" DVT_BOOT_ID_FILE="$boot_file" \
  ...
  --source-version 0.1.0-rc.14 --target-version 0.1.0-rc.15 \
...
jq -e '.script_version=="0.1.0-rc.15" and .state=="VERIFIED"' "$state_file" >/dev/null

printf 'rc.15 traffic-budget and migration checks passed\n'
