...
`experiments/htb-aggregate` 中的候选速率扫描是独立实验面，不进入六份 profile 或总控菜单。计划生成器只输出 JSON；runner 必须由 root 显式提供固定计划、获授权 iperf3 endpoint、固定 standalone profile、HTB v0.3.0 工具和新证据目录，且只对 200 Mbps 的 rc.12 schema 4 `VERIFIED` Debian 13 1C1G/1C2G 基线执行上传测试。runner 复用现有 HTB start/ACTIVE/stop/恢复契约，不创建持久化 qdisc。分析器只使用精确 sender bytes 归一化重传和 receiver goodput 形成描述性 shortlist，不推算丢包率或自动授权生产速率；候选仍须进入独立 A/B/A 与反序复验。

总控入口支持交互安全引导和显式 action。带宽只为 `guided`、`preflight`、`apply` 选择，默认 200 Mbps，范围是 100–1000 的任意整数；`apply` 的选择优先级是 `--port`、`PORT_SPEED_MBPS`、已安装状态值、交互选择/默认值，因而无显式值的重复 `apply` 复用已安装带宽；`verify`、`status`、`diagnose`、`benchmark`、`update`、`rollback` 不通过 `--port` 重写现有状态。安全引导先运行 preflight，只有交互终端再次明确确认才调用 apply。`diagnose` 只读取本机状态并做 1–60 秒增量采样，覆盖 TCP、softnet、整机 CPU、接口/ethtool、qdisc 和代理进程资源，不生成流量或输出进程命令行。`benchmark` 不修改系统配置，但只有用户显式给出 `BENCHMARK_HOST` 且已安装 iperf3 时才产生直连 TCP 测试流量；它允许固定预热、IPv4/IPv6、方向和 run ID，并按方向分离内核计数，仍不代表代理业务测试。测试前的 payload 流量估算只接受显式 `BENCHMARK_RATE_CAP_MBPS` 或合法管理状态中的端口上限，不使用 profile 默认值猜测；估算不含协议开销，也不等同于计费流量。普通菜单不提供 rc.2 专用 `recover`。
