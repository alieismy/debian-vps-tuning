...
其他拓扑在第一次系统写入前阻断。IPv4 和 IPv6 默认路由网卡分别发现并去重；策略路由表不属于该发现范围。`diagnose` 和 benchmark 会只读显示 policy rules，并在发现自定义规则时保存对应地址族的全部路由表和警告，但该证据不会把复杂策略路由提升为受支持的 apply 拓扑。
