...
| C19 | 用户授权的 `benchmark` | 仅向指定 iperf3 服务端执行 upload/download/both；并行 1–4；可固定 IPv4/IPv6；每方向保存原始 JSON、sender 吞吐/重传/每 GiB 重传、host-wide TCP/接口增量；qdisc 存在 leaf 时用 leaf、否则用 root；以 `INCOMPLETE → COMPLETED` 提交总结果及哈希链 | 结构化摘要、空/损坏 JSON、缺失/负计数器、mq leaf 和 manifest 失败终态 fixture 通过；目标 VPS 待测 |
