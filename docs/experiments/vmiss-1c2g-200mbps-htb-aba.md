...
状态：研究专用协议；默认不执行；Core/1C2G A/B/A 未执行
适用版本：本文保留的 rc.14/HTB 执行器 `0.4.0` 绑定仅作历史协议参考；当前候选须先按 rc.17 重新评审

> **效力边界：本文不是业务 VPS 的升级、发布或日常验收门禁。** 只有明确需要验证聚合出口
> 整形机制、使用独立高额度测试机和获授权 endpoint，并预先批准完整硬流量预算与停止条件
> 时，才可重新评审和执行。不得仅因主机为 1C2G/200 Mbps 就运行本文。

Basic 1C1G 的 HTB200 reference 已于 2026-09-11 完成，但不构成本 1C2G/Core A/B/A 的
前置授权或因果结论。当前仍没有 default `fq` 对照、candidate shortlist、Core reference
或真实 VLESS + REALITY + TCP 业务证据；不得因为 Basic reference 收尾而启动本文。
...
本 SOP 只适用于先在目标机另行完成对应候选版本生命周期验证的以下目标类别；当前文档状态仍为待目标机执行，不把历史 rc.12/rc.13/rc.14 运行记录继承为 rc.17 通过：
